#!/usr/bin/env python3
"""
rank_standard.py — STANDARD MODE (Original Approach)

Traditional ranking pipeline: score ALL candidates against the JD.

PIPELINE:
  Input: 100,000 candidates
    ↓
  Stage 0: Honeypot detection (filter 53 impossible profiles)
    ↓
  Stage 1: Title gate (multiplicative: 1.0 → 0.05)
    ↓
  Stage 2: Quality blend (7 components: skill, BM25, career-text, assessment, etc.)
    ↓
...
KEY CHARACTERISTICS:
  - Scores ALL 100,000 candidates
  - Computes BM25 for all 100K (expensive but comprehensive)
  - Every candidate gets a score, even low-relevance ones
  - Runtime: ~76 seconds on CPU
  - Guarantees finding any hidden gems in low-relevance pool
  - Higher compute cost, complete coverage

WHEN TO USE:
  - Need absolute certainty about top 100
  - Have compute budget (76s is fine for batch jobs)
  - Want to see full distribution of scores across pool
"""
...
def rank_candidates_standard(cands, top_n=100):
    """
    STANDARD MODE: Score all candidates.

    This is the original, comprehensive approach:
    1. Compute BM25 for ALL 100,000 candidates
    2. For each candidate:
       - Check if honeypot (skip if true)
       - Compute title gate
       - Compute 7-component quality blend
       - Apply engagement multiplier
       - Final score = title_gate * quality * engagement
    3. Sort by score (descending), candidate_id (ascending for ties)
    4. Return top 100

    TIME: ~76 seconds for 100,000 candidates
    COVERAGE: Every candidate is scored and ranked
    """
    print('[STANDARD MODE] Computing BM25 for all candidates...')
    bm25 = compute_bm25(cands)
    print(f'[STANDARD MODE] Scoring all {len(cands):,} candidates...')
...
def main():
    ap = argparse.ArgumentParser(description='Redrob Standard Ranker (STANDARD MODE)')
    ...
    ranked = rank_candidates_standard(cands, top_n=args.top)
